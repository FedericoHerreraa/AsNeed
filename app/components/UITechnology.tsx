        ...
        <Marquee>
            <div className="flex mt-20">
                {services.map((tech, index) => (
                    <div
                        key={index}
                        className='w-fit md:mx-3 mx-1 rounded-full text-center md:px-4 px-2 md:py-2 py-1 border duration-100 dark:bg-[rgba(13,28,44,0.6)] dark:border-zinc-700 dark:hover:border-zinc-500 bg-zinc-100 border-blue-400 hover:border-blue-700'
                    >
                        <p className='opacity-100 font-semibold bg-gradient-to-r md:text-sm text-xs dark:from-blue-300 dark:to-blue-800 from-blue-500 to-blue-950 bg-clip-text text-transparent'>{spanish ? tech.spanish : tech.english}</p>
                    </div>
                ))}
            </div>
        </Marquee>
